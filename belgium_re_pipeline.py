...
================================================================================
 ...
 Portfolio Project | Data Analyst: Ying
================================================================================

WHAT THIS MODEL DOES:
  This model estimates the fair market value of a residential property based on
  its location, size, type, and condition — so investors and agencies can
  identify over- and under-priced listings relative to market comparables.
...
RUN INSTRUCTIONS:
  1. Open a terminal / Anaconda Prompt in the folder containing this script.
  2. Run: python belgium_re_pipeline.py
  3. Two CSV files will be created in the same folder:
       • belgium_re_powerbi_export.csv
       • belgium_re_feature_importance.csv

NUMBER FORMAT NOTE:
  All printed figures use Dutch locale convention:
    • Period  (.)  = thousands separator
    • Comma   (,)  = decimal separator
  Example: € 353.750  |  € 3.333,33 / m²
================================================================================
"""
...
# Kitchen quality ordinal score (0 = not installed → 3 = hyper equipped)
kitchen_map = {
...
# Building condition ordinal score (0 = worst → 5 = as new)
state_map = {
...
print("""
MODEL PURPOSE:
  This model estimates the fair market value of a residential property based
  on its location, size, type, and condition — so investors and agencies can
  identify over- and under-priced listings relative to market comparables.
""")
...
print(f"  R²              : {r2:.3f}   (proportion of price variance explained)")
print(f"  MAE             : {dutch(mae)}   (mean absolute prediction error)")
print(f"  MAPE            : {mape:.1f}%   (mean absolute percentage error)")
print()
print("  NOTE ON R²: An R² of 0.635 means the model explains ~63,5% of price")
print("  variation using publicly observable listing features alone. Unobserved")
print("  factors (micro-location quality, interior finish, views) account for")
print("  the remainder — consistent with real estate valuation benchmarks.")
...
print("=" * 70)
print("""
SCENARIO: An investor buys a 'TO_RENOVATE' property in Flanders and renovates
  it to 'AS_NEW' condition. What is the expected return?

ASSUMPTIONS:
  [FROM DATA]   Median price TO_RENOVATE (Flanders)  : see below
  [FROM DATA]   Median price AS_NEW (Flanders)        : see below
  [ASSUMED]     Renovation cost per m²               : € 800
  [ASSUMED]     Median living area for scenario       : 170 m² (dataset median)
  [ASSUMED]     Transaction costs (notary, agent)     : 10% of purchase price
  [ASSUMED]     Holding period                        : 18 months
""")
...
med_area = 170  # assumed
reno_cost_m2 = 800  # assumed
reno_cost_total = med_area * reno_cost_m2
tx_cost = fl_reno * 0.10  # assumed 10%
...
print(f"  Transaction costs (10% — ASSUMED)            : {dutch(tx_cost)}")
print(f"  Renovation cost ({med_area} m² × {dutch(reno_cost_m2, prefix='€ ')}/m² — ASSUMED) : {dutch(reno_cost_total)}")
...
COLUMN NOTES (belgium_re_powerbi_export.csv)
  ...
  kitchen_score     : 0 = not installed → 3 = hyper equipped
  building_score    : 0 = to restore → 5 = as new
